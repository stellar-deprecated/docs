...
>
> The sequence number on an account is updated anytime the transaction fee is deducted from an account. In order for this to happen, the sequence number on the transaction needs to be valid, the transaction needs to have the correct signatures, and the time bounds condition needs to be met. If there are any obvious errors in the transaction that can be handled without checking the ledger (for example, negative amounts) then the sequence number is not incremented. If there is an insufficient balance for transaction fees then the sequence number is not incremented. In all other cases, the transaction fees is taken and the sequence number updated, including the case where there is an insufficient balance for a payment then the sequence number _is_ incremented.
>
> #### List of operations
