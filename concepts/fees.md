...
## Minimum account balance: the reserve amount

In order to prevent unbounded account creation, accounts must maintain a minimum XLM balance. The network will reject transactions if applying the transaction would reduce an account's balance to less than the minimum balance—e.g., via fees or balance transfer.

The minimum balance is affected by the number of entries attached to an account.
...
The minimum balance follows the following formula:
...
An account with 1 trustline and 2 offers would require a minimum balance of (2+3)*10,000,000 = 50,000,000 stroops = 50 XLM
